...
if debugX then
	warn('Initialising Aucor')
end

local requestsDisabled = getgenv and getgenv().DISABLE_Aucor_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.671"
local AucorFolder = "Aucor"
local ConfigurationFolder = AucorFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		AucorOpen = {Type = 'bind', Value = 'K', Name = 'Aucor Keybind'},
		-- buildwarnings
		-- Aucorprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(AucorFolder) then
				if isfile and isfile(AucorFolder..'/settings'..ConfigurationExtension) then
					file = readfile(AucorFolder..'/settings'..ConfigurationExtension)
				end
			end
	...
	if not success then 
		if writefile then
			warn('Aucor had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					...
					warn("Failed to load or execute the reporter. \nPlease notify Aucor developers at sirius.menu/discord.")
				end
			end)
...
local Aucor = useStudio and script.Parent:FindFirstChild('Aucor') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Aucor:FindFirstChild('Build') and Aucor.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Aucor | Build Mismatch')
		print('Aucor may encounter issues as you are running an incompatible interface version ('.. ((Aucor:FindFirstChild('Build') and Aucor.Build.Value) or 'No Build') ..').\n\nThis version of Aucor is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Aucor = Aucor, useStudio and script.Parent:FindFirstChild('Aucor') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Aucor)
	Aucor.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Aucor.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Aucor.Name and Interface ~= Aucor then
			Interface.Enabled = false
			Interface.Name = "Aucor-Old"
		end
	end
elseif not useStudio then
...
if Aucor.AbsoluteSize.X < minSize.X and Aucor.AbsoluteSize.Y < minSize.Y then
...
local Main = Aucor.Main
local MPrompt = Aucor:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Aucor:FindFirstChild('Drag')
...
Aucor.DisplayOrder = 100
LoadingFrame.Version.Text = Release


local Icons = useStudio and require(script.Parent.icons) or loadstring(game:HttpGet('https://raw.githubusercontent.com/SiriusSoftwareLtd/Aucor/refs/heads/main/icons.lua'))()
...
local Notifications = Aucor.Notifications

local SelectedTheme = AucorLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AucorLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Aucor.Main.BackgroundColor3 = SelectedTheme.Background
	Aucor.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Aucor.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Aucor.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Aucor.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Aucor.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Aucor.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Aucor.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Aucor.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Aucor:GetDescendants()) do
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Aucor had an issue decoding the configuration file, please try delete the file and reopen Aucor.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AucorLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Aucor | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AucorLibrary:Notify({Title = "Aucor Flags", Content = "Aucor was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AucorLibrary.Flags) do
...
function AucorLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Aucor | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AucorLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Aucor Hub'.", Duration = 7, Image = 4400697855})
		else
			AucorLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.AucorOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(AucorFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Aucor Settings', 0, true)

	if TabList['Aucor Settings'] then
		TabList['Aucor Settings'].LayoutOrder = 1000
	end

	if Elements['Aucor Settings'] then
		Elements['Aucor Settings'].LayoutOrder = 1000
	end
...
function AucorLibrary:CreateWindow(Settings)
	if Aucor:FindFirstChild('Loading') then
		if getgenv and not getgenv().AucorCached then
			Aucor.Enabled = true
			Aucor.Loading.Visible = true

			task.wait(1.4)
			Aucor.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AucorCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AucorLibrary:Notify({Title = 'Build Mismatch', Content = 'Aucor may encounter issues as you are running an incompatible interface version ('.. ((Aucor:FindFirstChild('Build') and Aucor.Build.Value) or 'No Build') ..').\n\nThis version of Aucor is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(AucorFolder) then
		makefolder(AucorFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Aucor"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Aucor Interface Suite" then
		LoadingFrame.Version.Text = "Aucor UI"
	end
	...
	if not Settings.DisableAucorPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AucorLibrary:Notify({
					Title = "Aucor Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(AucorFolder.."/Discord Invites") then
			makefolder(AucorFolder.."/Discord Invites")
		end

		if isfile and not isfile(AucorFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(AucorFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Aucor RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(AucorFolder.."/Key System") then
			makefolder(AucorFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Aucor | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Aucor specific development.')
				end
			end
		end
		...
		if isfile and isfile(AucorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(AucorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(AucorFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						AucorLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Aucor | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Aucor specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AucorLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AucorLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Aucor.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Aucor | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Aucor specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AucorLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Aucor | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Aucor specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AucorLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Aucor | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Aucor specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AucorLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AucorLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Aucor | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Aucor specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AucorLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Aucor | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Aucor specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AucorLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AucorLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AucorLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Aucor had an issue creating settings.') end
...
function AucorLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AucorLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AucorLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Aucor Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.AucorOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AucorLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AucorLibrary:Notify({Title = "Aucor Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AucorLibrary:Notify({Title = "Aucor Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Aucor Configurations Error | '..tostring(result))
			AucorLibrary:Notify({Title = "Aucor Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = AucorLibrary:CreateWindow({
		Name = "Aucor Example Window",
		LoadingTitle = "Aucor Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Aucor may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Aucor to get the key from
	...
	--AucorLibrary:Notify({Title = "Aucor Interface", Content = "Welcome to Aucor. These - are the brand new notification design for Aucor, with custom sizing and Aucor calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(AucorLibrary.Theme) do
...
task.delay(4, function()
	AucorLibrary.LoadConfiguration()
